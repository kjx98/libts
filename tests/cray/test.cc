...
{
	u8	hh[20];
	ASSERT_TRUE(sizeof(head0) == 16);
	memcpy(hh, headBytes, sizeof(head0));
	crayUDP_header *hp=nullptr;
	hp = hp->decodeInline(hh, sizeof(head0));
...
{
	crayUDP_header head;
	ASSERT_TRUE(head.decode(headBytes, sizeof(head)));
...
TEST(testMold, TestEncodeHead1)
{
	...
	ASSERT_TRUE(memcmp(buff, headBytes, sizeof(head0)) == 0);
}

TEST(testMold, TestEncodeHead2)
{
	crayUDP_header head1= head0;
	ASSERT_TRUE(head1.encodeInline());
	ASSERT_TRUE(memcmp(&head1, headBytes, sizeof(head0)) == 0);
}
